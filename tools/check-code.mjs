...
// ══════════════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════════════
// หน้าลูกค้าสแกนต้องเลื่อนดูเมนูได้บนมือถือ — สองเหตุจริง คนละต้นตอ
// ① 9 ก.ย. 69: กรอบใช้ min-height:100vh = สูงตามเนื้อหา (วัดจริง 11,499px) กล่องเมนูข้างในเลยไม่เคยล้น
//    จึงไม่มีแถบเลื่อน และกล่องนั้นเต็มจอ = ปัดนิ้วตรงไหนก็ไม่ขยับ → แก้ด้วยกรอบสูงเท่าจอ + กล่องข้างในเลื่อนเอง
// ② 20 ก.ย. 69: ลูกค้า Android เปิดผ่านเบราว์เซอร์ในแอป (LINE) ปัดดูเมนูไม่ได้อีก ทั้งที่บนเบราว์เซอร์ปกติเลื่อนได้
//    ต้นตอคือ "กล่องเลื่อนซ้อนในหน้าที่สูงเท่าจอพอดี" ซึ่งเบราว์เซอร์ในแอปจัดการไม่เหมือนกัน
//    ⟹ เลิกพึ่งกล่องเลื่อนซ้อน ให้ "ทั้งหน้า" เลื่อนแทน (การเลื่อนพื้นฐานที่สุด ทุกเบราว์เซอร์ทำเหมือนกันหมด)
//    หัวจอ/แถบหมวด/ช่องค้นหาใช้ sticky ให้ติดบน และแถบปุ่มใช้ sticky ให้ติดล่าง — ได้หน้าตาเดิมโดยไม่ต้องมีกล่องเลื่อน
// กติกาที่เหลือไว้: ห้ามมีกล่อง flex:1+overflow:auto ในหน้านี้อีก และกรอบต้องเป็น min-height ไม่ใช่ height
// ══════════════════════════════════════════════════════════════════════════
section("หน้าลูกค้าสแกน: เลื่อนดูเมนูได้");
{
  ...
  ok_("หน้าลูกค้าเลื่อนทั้งหน้า ไม่ใช่เลื่อนในกล่อง", CUST.includes('return <div className="cust-shell"'));
  ok_("กรอบสูงอย่างน้อยเท่าจอ และเผื่อเบราว์เซอร์ที่ไม่รู้จัก dvh",
    HTML.includes(".cust-shell { min-height: 100vh; min-height: 100dvh; }"));
  ck("ไม่มีกล่องเลื่อนซ้อนในหน้าลูกค้าอีก (ตัวที่เบราว์เซอร์ในแอปเอาไม่อยู่)",
...
try {
  ...
  // ① วันเดียว เงินสด+พร้อมเพย์ — พร้อมเพย์เป็นชั้นย่อย ห้ามบวกซ้ำ
  {
    ...
    // 23 ก.ย. 69 เปลี่ยนเป็นคีย์รายกะ: วันหนึ่งมีได้หลายใบ ใบละกะ (ร้านเปิดวันละ 2 กะ)
    // ก่อนหน้านี้ใบของกะที่สองชนคีย์ (วัน+สาขา) แล้วถูกทิ้งเงียบ ๆ รวม ฿46,421 ใน 5 วัน
    ck("external_id ผูกกับกะ สำหรับสาขาที่เปิดคีย์รายกะแล้ว", p.external_id, "pos-2026-09-09-กาญจนบุรี The River-s8");
    ck("ส่งเลขกะไปให้บัญชีด้วย", p.shift_id, 8);
    // ฝั่งบัญชีนับ "กะที่เท่าไหร่ของวัน" เองจากเวลาปิดกะ แล้วตรึงเลขไว้บนใบกำกับภาษี
    ck("ส่งเวลาเปิด/ปิดกะไปด้วย", { o: p.shift_opened_at, c: p.shift_closed_at },
      { o: "2026-09-08T06:00:00Z", c: "2026-09-09T17:00:00Z" });
...
section("วิธีจ่าย: บรรทัดแม่-ลูกที่ส่งเข้าบัญชี");
{
  ...
  if (build) {
    ...
    // เดิมช่องทาง "อื่นๆ (ไม่ระบุ)" ไม่มีบรรทัดลูก ⟹ ตกจากการแตกหมวดรายได้ฝั่งบัญชี (กติกา leaf)
    // ตอนนี้ต้องมีบรรทัดของตัวเองเสมอ: แม่ Custom Payment แล้วตามด้วยลูกชื่อเดียวกับที่เขาแมปไว้
    ok_("ช่องทางไม่ระบุมีทั้งบรรทัดแม่และลูก",
      only.payment.length === 3 && only.payment[1].name_th === "Custom Payment"
      && only.payment[2].name_th === "ช่องทางอื่น (ไม่ระบุ)" && only.mainSum === 120);
    ck("ช่องทางไม่ระบุ: ผลรวมแบบ leaf ครบเท่ายอดจริง", only.leafSum, 120);
...
section("แบ่งจ่ายหลายช่องทาง");
{
  ...
  // ชื่อสองฟีเจอร์ต้องไม่ชวนสับสน: "แบ่งจ่าย" = จ่ายจริงหลายช่องทาง · อีกอันแค่พิมพ์ใบตัวอย่างให้ลูกค้าหารกันเอง
  ok_("ใบตัวอย่างหารกันเอง ยังเข้าถึงได้ และชื่อไม่ชนกับแบ่งจ่าย",
    APP.includes("พิมพ์ใบตัวอย่างหารกันเอง (ไม่ใช่การชำระเงิน)") && !APP.includes("แบ่งบิลตามคน"));
...
// ══════════════════════════════════════════════════════════════════════════
// จอขายหน้าร้านต้อง "อ่านออกและกดถูก" ตอนร้านแน่น (เจ้าของสั่งทำให้สะอาด 21 ก.ย. 69)
// เดิมมีตัวหนังสือเล็กถึง 9px และปุ่มเพิ่ม-ลดจำนวนแค่ 28px ซึ่งเป็นปุ่มที่กดบ่อยที่สุดตอนพีค
// พนักงานยืนอ่านจอห่างแขน ตัวเล็กกว่านี้คืออ่านผิดแล้วคิดเงินผิด
// ══════════════════════════════════════════════════════════════════════════
section("จอขาย: อ่านออก กดถูก");
{
  const seg = (name) => {
    const lines = APP.split("\n");
    const a = lines.findIndex((l) => l.startsWith("function " + name + "("));
    if (a < 0) return "";
    const b = lines.findIndex((l, i) => i > a && /^function [A-Z]/.test(l));
    return lines.slice(a, b < 0 ? lines.length : b).join("\n");
  };
  const sizesIn = (txt) => [...txt.matchAll(/fontSize:(\d+(?:\.\d+)?)/g)].map((m) => +m[1]);
  for (const [name, floor] of [["POSSaleMode", 12], ["POSOrderPanel", 12], ["PayModal", 12]]) {
    const txt = seg(name);
    ok_("อ่าน " + name + " ได้", !!txt);
    if (txt) ck("ตัวหนังสือใน " + name + " ไม่เล็กกว่า " + floor, sizesIn(txt).filter((v) => v < floor), []);
  }
  // การ์ดโต๊ะกว้าง 116 สูง 96 — ขยายได้แต่ไม่เท่าจออื่น ไม่งั้นตัวหนังสือล้นการ์ด
  ck("ตัวหนังสือในผังโต๊ะไม่เล็กกว่า 11", sizesIn(seg("POSTableMap")).filter((v) => v < 11), []);
  // ปุ่มที่นิ้วต้องกดในจอขาย ต้องไม่เล็กกว่า 44 (มาตรฐานนิ้วผู้ใหญ่)
  ok_("ปุ่มเพิ่ม-ลดจำนวนในแผงออเดอร์ใหญ่พอสำหรับนิ้ว",
    APP.includes('<button onClick={()=>chQty(idx,-1)} aria-label="ลดจำนวน" style={{width:44,height:44,')
    && APP.includes('<button onClick={()=>chQty(idx,1)} aria-label="เพิ่มจำนวน" style={{width:44,height:44,'));
...
// ══════════════════════════════════════════════════════════════════════════
// เงินสดต้องเข้ากะที่ "เปิดอยู่จริงในฐาน" เสมอ — ห้ามเชื่อกะที่ค้างอยู่ในหน้าจอ
// เหตุจริง 22 ก.ย. 69 (The River): เครื่องแคชเชียร์เปิดค้างข้ามคืน ยังถือกะ #23 ที่ปิดไปแล้ว
// เงินสดทั้งวัน ฿272 (บิล #372 #374 #375) ถูกบันทึกเข้ากะที่ปิดแล้ว
// กะ #24 ที่เปิดจริงจึงขึ้น "ขายเงินสด ฿0" และตอนปิดกะเงินในลิ้นชักเกิน ฿272 แบบไม่มีที่มา
// เจ้าของย้ำ: เงินสดในระบบกับของจริงไม่ตรง = พนักงานโดนหักเงิน ⟹ ห้ามพลาดเด็ดขาด
// ══════════════════════════════════════════════════════════════════════════
section("เงินสดต้องเข้ากะที่เปิดอยู่จริง");
{
  const grabB = (head) => {
    const st = APP.indexOf(head); if (st < 0) return "";
    let d = 0, started = false;
  ...
  const body = grabB("async function liveShift(branchId,stateShift){");
  ok_("มีตัวหากะที่เปิดอยู่จริง", !!body);
  if (body) {
    const mk = (live) => new Function("api", body + "\nreturn liveShift;")({ getActiveShift: async () => (live ? [live] : []) });
    const run = async (live, stateShift) => await mk(live)(8, stateShift);
    // จอถือกะเดียวกับที่เปิดอยู่จริง = เงียบ ไม่ต้องกวนพนักงาน
    await run({ id: 24 }, { id: 24 }).then((r) => ck("กะตรงกัน = บันทึกเข้ากะนั้น ไม่เตือน", { id: r.id, note: r.note }, { id: 24, note: "" }));
    // จอค้างกะเก่า = ต้องเขียนเข้ากะที่เปิดจริง และบอกพนักงานให้รีเฟรช
    await run({ id: 24 }, { id: 23 }).then((r) => {
      ck("จอค้างกะเก่า = เงินเข้ากะที่เปิดจริง", r.id, 24);
      ok_("และต้องเตือนพนักงาน ไม่ใช่เงียบ", !!r.note && r.note.includes("#24") && r.note.includes("#23"));
    });
    // ไม่มีกะเปิดเลย = ยังต้องเก็บเงินไว้ ห้ามทิ้ง แต่ต้องเตือนดังๆ
    await run(null, { id: 23 }).then((r) => {
      ck("ไม่มีกะเปิด = ไม่ทิ้งเงิน ใช้กะล่าสุดของจอไว้ก่อน", r.id, 23);
      ok_("พร้อมเตือนให้ไปเปิดกะและแจ้งผู้จัดการ", !!r.note && r.note.includes("ไม่มีกะเปิดอยู่"));
    });
    // อ่านฐานไม่ได้ (เน็ตสะดุด) ก็ต้องไม่พัง
    await new Function("api", body + "\nreturn liveShift;")({ getActiveShift: async () => { throw new Error("เน็ตสะดุด"); } })(8, { id: 23 })
      .then((r) => ck("อ่านฐานไม่ได้ = ใช้กะของจอ ไม่โยน error", r.id, 23));
  }
  // ทุกจุดที่เขียนเงินสด ต้องผ่านตัวหากะที่เปิดจริง — ยกเว้นเปิดกะ/ปิดกะซึ่งผูกกับกะของตัวเองอยู่แล้ว
...
section("ท่อบัญชี: กติกา leaf + คีย์รายกะ");
{
  ...
  if (bpl) {
    ...
    ck("ช่องทางไม่ระบุปนไทยช่วยไทย: ชั้นพ่อครบ", trapOther.mainSum, 120);
    // เคยเป็นรูรั่ว — ตอนนี้ "ช่องทางอื่น (ไม่ระบุ)" มีบรรทัดลูกของตัวเองแล้ว ยอดจึงครบทั้งสองแบบ
    ck("ช่องทางไม่ระบุปนไทยช่วยไทย: leaf ครบแล้ว", trapOther.leafSum, 120);
    ok_("ช่องทางไม่ระบุมีบรรทัดของตัวเอง (ฝั่งบัญชีแมปเข้า 41100.09.10)",
      trapOther.payment.some(p => p.name_th === "ช่องทางอื่น (ไม่ระบุ)"));
    ...
    ck("ช่องทางไม่ระบุล้วน: leaf ครบ", bpl([{ payment_method: "other", total: 70 }]).leafSum, 70);
  }
  // ฝั่งบัญชีขอ 23 ก.ย. 69: บล็อกเฉพาะเคส "ตัวเลขไม่ตรง" เท่านั้น
  // เคสชื่อบรรทัดไม่มีในผังบัญชีเขา ไม่ต้องบล็อก เพราะเงินไม่หาย (ตกไปลงหมวดหลักของสาขา)
  ok_("leaf ไม่ตรงยอดขาย = บล็อกไม่ยิง (ลงผิดแก้ยากกว่าไม่ลง)",
    SLIPPUSH.includes("if (Math.abs(leafSum - total_sales) > 0.005)"));
  ok_("ทุกช่องทางในจอขายมีบรรทัด leaf ของตัวเอง ยกเว้นบัตรที่ยังไม่มีในจอ",
    SLIPPUSH.includes('{ pm: "other", name_th: "ช่องทางอื่น (ไม่ระบุ)"'));
  ...
  // เปิดใช้จริง 23 ก.ย. 69 23:45 น. หลังฝั่งบัญชีเปลี่ยนคีย์ตัวรับ (วัน+สาขา+กะ) และเติมรหัสกะย้อนหลังครบ 15 ใบ
  // รายชื่อนี้ต้องมีเฉพาะสาขาที่ฝั่งบัญชีเปิดคีย์ให้จริงเท่านั้น — เติมสาขาอื่นก่อนเขาพร้อม
  // = ใบปิดยอดของทั้งวันกลายเป็นใบที่ตัวรับไม่รู้จัก แย่กว่าอาการเดิมที่แก้กันอยู่
  ok_("คีย์รายกะเปิดเฉพาะ The River ตามที่นัด cutover กันไว้",
    SLIPPUSH.includes("const SHIFT_KEYED_BRANCHES = [8];"));
