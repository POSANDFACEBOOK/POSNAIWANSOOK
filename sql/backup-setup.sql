...
-- 0) crm_line_users has no single unique id, which keyset-paginated backup needs (paginating on a
--    non-unique column would silently drop rows). Give it a surrogate id. Safe on the live table:
--    the webhook insert doesn't set id (auto-filled) and its (branch_id,line_user_id) conflict key
--    is unchanged.
alter table public.crm_line_users add column if not exists id bigint generated by default as identity;
create unique index if not exists crm_line_users_id_key on public.crm_line_users (id);
