...
async function loadOpening(period) {
  try {
    const rows = await sbFetchAll(`sliptrack_opening?select=id,branch,account,value&period=eq.${encodeURIComponent(period)}`);
    const m = new Map();
    for (const r of rows) m.set(`${r.branch}|${r.account}`, +r.value || 0);
    return m;
  ...
  try {
    ...
    if (only !== "assets") {
      ...
      if (opening.size > 0) for (const p of payloads) {
        const v = opening.get(`${p.branch}|${p.category}`);
        if (v != null) p.opening = round2(v);
      }
