...
// ══════════════════════════════════════════════════════════════════════════
import { accountFor, branchNameForAccounting, ALL_ACCOUNTS } from "./sliptrack-map.js";
import { assetPayload, disposePayload } from "./sliptrack-assets.js";
...
// ── ยอดยกมา (opening) ของงวดแรก ────────────────────────────────────────────
// บัญชีคิด "ใช้ไป = ยกมา + ซื้อ − ตรวจนับปลายงวด" งวดแรกจึงต้องมียกมา ไม่งั้นต้นทุนต่ำกว่าจริง
// ทั้งก้อน. ค่านี้ต้องนิ่ง (ส่งซ้ำทุกวันต้องได้เลขเดิม) จึงเก็บไว้ในตาราง sliptrack_opening
// ครั้งเดียว แล้วอ่านมาแนบทุกครั้งที่ส่ง — งวดถัดไปบัญชียกยอดตรวจนับงวดก่อนให้เอง
async function loadOpening(period) {
  try {
    const rows = await sbFetchAll(`sliptrack_opening?select=id,branch,account,value,captured_at&period=eq.${encodeURIComponent(period)}`);
    const m = new Map();
    for (const r of rows) m.set(`${r.branch}|${r.account}`, { value: +r.value || 0, capturedAt: r.captured_at });
    return m;
  } catch { return new Map(); }   // ยังไม่ได้สร้างตาราง = ไม่มียกมา (ไม่ทำให้งานหลักล้ม)
}

// หมายเหตุกำกับยอดยกมางวดแรก — ผู้สอบบัญชีต้องเห็นว่านี่เป็น "ยอดตั้งต้นระบบ" ไม่ใช่ยอด
// ตรวจนับ ณ วันแรกของงวด (log การเคลื่อนไหวสต๊อกเพิ่งเริ่มเก็บ 23 ก.ค. จึงย้อนไปต้นเดือนไม่ได้)
// การเปิดเผยข้อมูลตรงนี้จำเป็นต่อความถูกต้องของรายงาน
function openingNote(capturedAt) {
  let d = "";
  try { d = new Date(capturedAt).toLocaleDateString("en-GB", { timeZone: "Asia/Bangkok" }).replace(/\//g, "/"); } catch { d = ""; }
  return `ยอดตั้งต้นระบบ${d ? ` ณ ${d}` : ""} — ไม่ใช่ยอดตรวจนับวันแรกของงวด (log สต๊อกเริ่มเก็บ 23 ก.ค. 2026)`;
}

// จับภาพมูลค่าปัจจุบันเป็น "ยอดยกมา" ของงวดนี้ — ทำครั้งเดียว ไม่ทับของเดิมเด็ดขาด
// (ถ้าทับได้ ยอดยกมาจะเปลี่ยนทุกครั้งที่ยิง ซึ่งทำให้ต้นทุนที่คำนวณไว้แล้วเพี้ยนย้อนหลัง)
async function captureOpening(period, payloads) {
  const existing = await loadOpening(period);
  if (existing.size > 0) return { captured: 0, skipped: "มีอยู่แล้ว" };
  const rows = payloads.map((p) => ({ period, branch: p.branch, account: p.category, value: p.closing, note: `จับภาพอัตโนมัติ ${bkkToday()}` }));
  const r = await fetch(`${SUPA_URL}/rest/v1/sliptrack_opening`, {
    method: "POST",
    headers: { ...H, Prefer: "return=minimal,resolution=ignore-duplicates" },
    body: JSON.stringify(rows),
  });
  if (!r.ok) throw new Error(`บันทึกยอดยกมาไม่สำเร็จ: ${r.status} ${await r.text()}`);
...
export async function buildInventoryPayloads() {
  ...
  for (const b of branches) {
    ...
    if (!bName) continue;
    for (const ing of ings) {
      const { account } = accountFor(ing);
      // หมวดที่ยังไม่ได้จับคู่จะได้ "วัตถุดิบอื่นๆ" มาแทน (นับเข้ารายงาน + ถูกรายงานเตือนไว้ข้างบนแล้ว)
      // ข้ามเฉพาะที่ตั้งใจไม่ส่งจริงๆ (E01–E03 อุปกรณ์ใช้งานยาว)
      if (!account) continue;
...
// เรียกจาก api/sliptrack-sweep.js ด้วย ?job=daily — ผู้เรียกตรวจสิทธิ์และหา apiKey มาให้แล้ว
// (แพลน Vercel ปัจจุบันจำกัด 12 serverless function ต่อ deployment จึงรวมงานนี้ไว้กับ sweep
//  แทนที่จะเป็น endpoint แยก ไม่งั้น deploy ทั้งชุดล้มทันที)
export async function runDaily(req, res, apiKey) {
  ...
  try {
    ...
    if (only !== "assets") {
      ...
      // ?capture_opening=1 → จับภาพมูลค่าตอนนี้เป็น "ยอดยกมา" ของงวดนี้ (ทำครั้งเดียวก่อนเปิดใช้จริง)
      let openingInfo = null;
      if (String((req.query && req.query.capture_opening) || "") === "1" && !dry) {
        try { openingInfo = await captureOpening(period, payloads); }
        catch (e) { openingInfo = { error: (e && e.message) || String(e) }; }
      }
      // แนบยอดยกมาให้งวดที่มีบันทึกไว้ (ปกติคืองวดแรกงวดเดียว) — งวดหลังบัญชียกมาให้เอง
      const opening = await loadOpening(period);
      if (opening.size > 0) for (const p of payloads) {
        const o = opening.get(`${p.branch}|${p.category}`);
        if (o) { p.opening = round2(o.value); p.note = openingNote(o.capturedAt); }
      }
      ...
      out.stock = {
        ...
        openingAttached: opening.size, ...(openingInfo ? { openingCapture: openingInfo } : {}),
        errors,
