...
function Modal({title,onClose,children,wide,extraWide,noScroll,disableEsc}){
  const mob=useIsMobile();
  // ESC closes (unless disableEsc=true). Backdrop click is still ignored — only × or ESC.
  useEffect(()=>{
...
function ImportIngModal({onClose,ingCats,suppliers,currentUser,currentBranch,ings=[],onDone}){
  ...
  const[result,setResult]=useState({added:0,updated:0,failed:0});
  ...
  return <Modal title="📥 Import วัตถุดิบ" onClose={onClose} extraWide>
    ...
    {step===3&&<div style={{textAlign:"center",padding:"40px 0"}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap",margin:"14px auto 22px",maxWidth:420}}>
        <div style={{flex:"1 1 100px",background:C.greenLight,border:`1px solid ${C.green}55`,borderRadius:10,padding:"10px 12px"}}>
          <div style={{fontSize:11,color:C.ink3,fontFamily:"'Sarabun',sans-serif"}}>เพิ่มใหม่</div>
          <div style={{fontSize:22,fontWeight:900,color:C.green,fontFamily:"'Sarabun',sans-serif"}}>{result.added}</div>
        </div>
        <div style={{flex:"1 1 100px",background:C.blueLight,border:`1px solid ${C.blue}55`,borderRadius:10,padding:"10px 12px"}}>
          <div style={{fontSize:11,color:C.ink3,fontFamily:"'Sarabun',sans-serif"}}>อัพเดท</div>
          <div style={{fontSize:22,fontWeight:900,color:C.blue,fontFamily:"'Sarabun',sans-serif"}}>{result.updated}</div>
        </div>
        {result.failed>0&&<div style={{flex:"1 1 100px",background:C.redLight,border:`1px solid ${C.red}55`,borderRadius:10,padding:"10px 12px"}}>
          <div style={{fontSize:11,color:C.ink3,fontFamily:"'Sarabun',sans-serif"}}>ล้มเหลว</div>
          <div style={{fontSize:22,fontWeight:900,color:C.red,fontFamily:"'Sarabun',sans-serif"}}>{result.failed}</div>
        </div>}
      </div>
...
function ImportMenuModal({onClose,menuCats,currentUser,currentBranch,menus=[],onDone}){
    ...
    r.onload=ev=>{
      try{
        ...
        const parsed=json.map(row=>{
          ...
          return{_kid:randId(),code,name,category:allMenuCatList.includes(category)?category:defaultCat,price,description,selected:!!name};
        }).filter(r=>r.name);
        if(parsed.length===0){alert("ไม่พบรายการในไฟล์ — กรุณาตรวจสอบหัวคอลัมน์");return;}
        setRows(parsed);setStep(2);
      }catch(err){alert("อ่านไฟล์ไม่สำเร็จ: "+err.message);}
  ...
  function parseMenuText(text){
    ...
    lines.forEach(line=>{
      ...
      const desc=parts[3]||"";
      parsed.push({_kid:randId(),name,price,category:allMenuCatList.includes(cat)?cat:defaultCat,description:desc,selected:true});
    });
    return parsed.filter(r=>r.name&&r.name.length>=2);
  }
    ...
    for(const o of src){
      ...
      for(const it of (o.items||[])){
        ...
        out.push({
          ...
          qty,
          unit:it.unit||ing.buy_unit||"หน่วย",
          price,
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Sarabun',sans-serif",fontSize:13,minWidth:560}}>
          <thead>
            <tr style={{background:C.bg,borderBottom:`2px solid ${C.lineLight}`}}>
              ...
              <th style={{padding:"8px 10px",textAlign:"right",fontSize:11,fontWeight:800,color:C.ink3,letterSpacing:.3,width:110}}>รวม</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r,i)=>{
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(310px,100%),1fr))",gap:14}}>
        {paged.map(item=><Card key={item.id} hover style={{overflow:"hidden"}}>
            ...
            <div style={{flex:1,padding:"12px 14px 10px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
                ...
                <div style={{display:"flex",gap:4}}>
                  {canE&&<button aria-label="แก้ไข" onClick={()=>{setForm({name:item.name,code:item.code||"",category:item.category,buy_unit:item.buy_unit,buy_amount:item.buy_amount,buy_price:item.buy_price,convert_to_gram:item.convert_to_gram,price_per_gram:item.price_per_gram,stock:item.stock,safety_stock:branchSafety(item,currentBranch?.id)||"",image:item.image,note:item.note||"",supplier_id:String(item.supplier_id||""),supplier_name:item.supplier_name||"",has_sop:!!item.has_sop,sop:Array.isArray(item.sop)?item.sop:[],ingredients:Array.isArray(item.ingredients)?item.ingredients:[]});setEditId(item.id);setOpen(true);}} style={{background:C.blueLight,border:"none",borderRadius:8,padding:0,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",minWidth:38,minHeight:38}}><Ic d={I.pencil} s={15} c={C.blue}/></button>}
                  {canD&&<button aria-label="ลบ" onClick={()=>del(item.id,item.name)} style={{background:C.redLight,border:"none",borderRadius:8,padding:0,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",minWidth:38,minHeight:38}}><Ic d={I.trash} s={15} c={C.red}/></button>}
                </div>
              </div>
              {isCentral
          ...
          <div style={{padding:"10px 14px 14px",borderTop:`1px solid ${C.lineLight}`}}>
            ...
            {(()=>{const r=priceRangeByIng.get(+item.id);const fmt=n=>(+n).toLocaleString(undefined,{minimumFractionDigits:n%1===0?0:2,maximumFractionDigits:2});const same=r&&Math.abs(r.min-r.max)<0.005;const hasHist=!!r;const bg=hasHist?"#FFFBEB":"#F8FAFC";const bd=hasHist?"#FDE68A":C.line;const txt=hasHist?"#92400E":C.ink3;return <button type="button" onClick={()=>setPriceHistoryItem(item)} title="ดูประวัติราคา + ช่วงเวลาที่เคยซื้อ" style={{display:"flex",alignItems:"center",gap:6,fontSize:11,marginBottom:6,padding:"4px 9px",background:bg,border:`1px solid ${bd}`,borderRadius:8,fontFamily:"'Sarabun',sans-serif",flexWrap:"wrap",cursor:"pointer",width:"100%",textAlign:"left",transition:"box-shadow .15s"}} onMouseEnter={e=>e.currentTarget.style.boxShadow=`0 0 0 2px ${bd}66`} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
              <span style={{color:txt,fontWeight:700}}>💰 ราคาซื้อจริง</span>
              {hasHist
                ?(same
                  ?<b style={{color:txt,fontSize:13}}>฿{fmt(r.min)}/{item.buy_unit||"หน่วย"}</b>
                  :<><b style={{color:txt,fontSize:13}}>฿{fmt(r.min)}</b><span style={{color:txt}}>–</span><b style={{color:txt,fontSize:13}}>฿{fmt(r.max)}</b><span style={{color:txt,fontSize:11}}>/{item.buy_unit||"หน่วย"}</span></>)
...
function MenuTab({menus,reload,ings,menuCats,currentUser,currentBranch,addH,printers=[],branches=[],allCats=[],reloadCats}){
  ...
  // Add (or merge) an ingredient row from the ni inputs. De-dupes by
  // ingredientId — adding the same ingredient again accumulates grams
  // into the existing row instead of appending a duplicate line.
  function addIngredientToForm(){
    const id=+ni.ingredientId;
    const grams=+ni.amountGram;
    if(!id||!(grams>0)){alert("กรุณาเลือกวัตถุดิบและใส่จำนวนกรัมที่มากกว่า 0");return;}
    setForm(f=>{
      const list=(f.ingredients||[]).slice();
      const idx=list.findIndex(x=>+x.ingredientId===id);
      if(idx>=0)list[idx]={...list[idx],amountGram:(+list[idx].amountGram||0)+grams};
      else list.push({ingredientId:id,amountGram:grams});
        ...
        <div>
          ...
          <div style={{background:C.bg,borderRadius:12,padding:"12px",marginBottom:10,border:`1px solid ${C.line}`}}>
            ...
            <div style={{maxHeight:160,overflowY:"auto"}}>
              {(ingQ||"").trim()===""
                ?<div style={{fontSize:12,color:C.ink4,textAlign:"center",padding:"16px 8px",fontFamily:"'Sarabun',sans-serif"}}>🔍 พิมพ์ชื่อวัตถุดิบเพื่อค้นหา</div>
                :filteredIngs.length===0
                  ?<div style={{fontSize:12,color:C.ink4,textAlign:"center",padding:"12px 8px",fontFamily:"'Sarabun',sans-serif"}}>ไม่พบวัตถุดิบ "{ingQ}"</div>
                  :filteredIngs.map(ing=>{
                    const already=(form.ingredients||[]).find(x=>x.ingredientId===ing.id);
                    const ppg=(+ing.avg_price_per_gram>0?+ing.avg_price_per_gram:+ing.price_per_gram)||0;
                    return <div key={ing.id} onClick={()=>{if(!already)setNi(n=>({...n,ingredientId:String(ing.id)}));}} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,marginBottom:3,background:already?C.greenLight:ni.ingredientId===String(ing.id)?C.brandLight:C.white,border:`1px solid ${already?C.green:ni.ingredientId===String(ing.id)?C.brandBorder:C.line}`,cursor:already?"default":"pointer"}}>
                      ...
                      <span style={{fontSize:11,color:C.brand}}>฿{ppg.toFixed(3)}/g</span>
    ...
    {step==="preview"&&parsed&&<div>
      ...
      <div style={{maxHeight:320,overflowY:"auto",border:`1px solid ${C.line}`,borderRadius:10,marginBottom:14}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Sarabun',sans-serif"}}>
          <thead>
            <tr style={{position:"sticky",top:0,background:"#0F172A",zIndex:1}}>{["#","เมนู","หมวด","จำนวน","ราคาเฉลี่ย","ยอดสุทธิ"].map((h,i)=><th key={h} style={{padding:"9px 10px",textAlign:i>=3?"right":"left",fontSize:11,fontWeight:700,color:"#F8FAFC",position:"sticky",top:0,background:"#0F172A"}}>{h}</th>)}</tr>
          </thead>
          <tbody>
  ...
  return <div>
    ...
    {loading?<Loading text="โหลดยอดขาย..."/>:rows.length===0?<Card style={{padding:50,textAlign:"center"}}>
      ...
      <div style={{overflowX:"auto",maxHeight:560}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Sarabun',sans-serif"}}>
          <thead><tr style={{background:"#0F172A",position:"sticky",top:0,zIndex:2}}>
            <th style={{padding:"11px 12px",textAlign:"left",fontSize:11,color:"#F8FAFC",fontWeight:700,whiteSpace:"nowrap",position:"sticky",left:0,top:0,background:"#0F172A",zIndex:3,minWidth:200}}>เมนู</th>
            <th style={{padding:"11px 12px",textAlign:"left",fontSize:11,color:"#F8FAFC",fontWeight:700,whiteSpace:"nowrap",position:"sticky",top:0,background:"#0F172A"}}>หมวด</th>
            {dates.map(d=><th key={d} style={{padding:"11px 10px",textAlign:"center",fontSize:11,color:"#F8FAFC",fontWeight:700,whiteSpace:"nowrap",position:"sticky",top:0,background:"#0F172A"}}>{d.slice(5)}</th>)}
            <th style={{padding:"11px 12px",textAlign:"right",fontSize:11,color:"#F8FAFC",fontWeight:800,background:"#1E293B",whiteSpace:"nowrap",position:"sticky",top:0}}>จำนวน</th>
            <th style={{padding:"11px 12px",textAlign:"right",fontSize:11,color:"#F8FAFC",fontWeight:800,background:"#1E293B",whiteSpace:"nowrap",position:"sticky",top:0}}>ยอดขาย (฿)</th>
            {showCost&&<>
              <th style={{padding:"11px 12px",textAlign:"right",fontSize:11,color:"#FCA5A5",fontWeight:800,background:"#7F1D1D",whiteSpace:"nowrap",position:"sticky",top:0}}>ต้นทุน</th>
              <th style={{padding:"11px 12px",textAlign:"right",fontSize:11,color:"#A7F3D0",fontWeight:800,background:"#064E3B",whiteSpace:"nowrap",position:"sticky",top:0}}>กำไร</th>
              <th style={{padding:"11px 12px",textAlign:"right",fontSize:11,color:"#A7F3D0",fontWeight:800,background:"#064E3B",whiteSpace:"nowrap",position:"sticky",top:0}}>%</th>
            </>}
          </tr></thead>
          <tbody>
      ...
      <div style={{overflowX:"auto",maxHeight:560}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Sarabun',sans-serif"}}>
          <thead><tr style={{background:"#0F172A",position:"sticky",top:0,zIndex:1}}>
            {(showCost?["วันที่","สาขา","เมนู","หมวด","จำนวน","ราคา/หน่วย","ยอดสุทธิ","ต้นทุน","กำไร"]:["วันที่","สาขา","เมนู","หมวด","จำนวน","ราคา/หน่วย","ยอดสุทธิ"]).map((h,i)=><th key={h} style={{padding:"11px 12px",textAlign:i>=4?"right":"left",fontSize:11,fontWeight:700,color:"#F8FAFC",whiteSpace:"nowrap",position:"sticky",top:0,background:"#0F172A"}}>{h}</th>)}
          </tr></thead>
          <tbody>
            {filtered.map((r,i)=>{
              const matched=findMenu(r.menu_name);
...
const PO_STATUS={
  ...
  // Central accepted the request; document direction is correct (central → branch)
  // but stock has NOT moved yet. Central can print and then press "🚚 จัดส่ง".
  open:            {label:"📋 รอจัดส่ง",       short:"รอจัดส่ง",    color:"#F59E0B",bg:"#FEF3C7"},
  // Stock has moved from central → branch. Branch can press "ตรวจรับ".
  shipped:         {label:"🚚 จัดส่งแล้ว",     short:"จัดส่งแล้ว",  color:"#0EA5E9",bg:"#E0F2FE"},
...
function POSection({branches,ings,currentBranch,currentUser,reloadIngs,onOpenOrders,orders=[],reloadOrders}){
  ...
  async function acceptRequest(po){
    ...
    setConfirming(po.id);
    try{
      // Swap from/to + flip status to open. NO stock move yet — wait for "🚚 จัดส่ง".
  ...
  async function delPO(po){
    // Hard stop: never let a moved-stock PO be deleted, even if the button
    // somehow re-appears (race, stale DOM, scripted click).
    if(!DELETABLE_PO_STATUSES.has(po.status)){
      const st=PO_STATUS[po.status]||{label:po.status};
      alert(`ไม่สามารถลบเอกสารนี้ได้\n\nสถานะปัจจุบัน: ${st.label}\nสต๊อกถูกตัดไปแล้ว — ถ้าต้องการลบจริงๆ ให้กด "❌ ยกเลิก" ก่อน ระบบจะคืนสต๊อกอัตโนมัติ แล้วจะลบได้`);
      return;
    }
...
function POFormPage({branch,fromBranch,editPO,ings,currentUser,onClose,onSaved,reloadIngs}){
  ...
  const[vatPct,setVatPct]=useState(()=>{
    if(!editPO)return 0;
    // Prefer stored vat_rate (% directly). Fall back to back-computing from vat/subtotal if both are valid.
    if(Number.isFinite(+editPO.vat_rate)&&+editPO.vat_rate>0)return +editPO.vat_rate;
    const sub=+editPO.subtotal,vat=+editPO.vat;
    if(Number.isFinite(sub)&&sub>0&&Number.isFinite(vat))return +((vat/sub)*100).toFixed(2);
    return 0;
  });
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Sarabun',sans-serif"}}>
        <thead>
          <tr style={{background:C.bg,position:"sticky",top:0,zIndex:1}}>
            <th style={{padding:"8px 10px",textAlign:"center",fontSize:11,color:C.ink3,fontWeight:700,width:40,position:"sticky",top:0,background:C.bg}}>#</th>
            <th style={{padding:"8px 10px",textAlign:"left",fontSize:11,color:C.ink3,fontWeight:700,position:"sticky",top:0,background:C.bg}}>รายการ</th>
            <th style={{padding:"8px 10px",textAlign:"center",fontSize:11,color:C.ink3,fontWeight:700,width:80,position:"sticky",top:0,background:C.bg}}>หน่วย</th>
            <th style={{padding:"8px 10px",textAlign:"center",fontSize:11,color:C.ink3,fontWeight:700,width:90,position:"sticky",top:0,background:C.bg}}>จำนวน</th>
            <th style={{padding:"8px 10px",textAlign:"center",fontSize:11,color:C.ink3,fontWeight:700,width:120,position:"sticky",top:0,background:C.bg}}>ราคา/หน่วย</th>
            <th style={{padding:"8px 10px",textAlign:"right",fontSize:11,color:C.ink3,fontWeight:700,width:120,position:"sticky",top:0,background:C.bg}}>รวม</th>
            <th style={{width:36,position:"sticky",top:0,background:C.bg}}></th>
          </tr>
        </thead>
        <tbody>
          ...
          {items.map((it,idx)=><tr key={it.ingredient_id||it.ingredientId||`${it.name}-${idx}`} style={{borderTop:`1px solid ${C.lineLight}`}}>
...
function StockCheckView({ings,suppliers,branches=[],currentBranch,currentUser,reload,reloadIngs}){
  ...
  return <div>
    {/* Hero header */}
    <div style={{background:`linear-gradient(135deg,#0F172A 0%,#1E293B 60%,${C.brand} 140%)`,borderRadius:16,padding:"18px 22px",marginBottom:14,color:"#F8FAFC",boxShadow:"0 6px 20px rgba(15,23,42,0.18)",position:"relative",overflow:"hidden"}}>
    ...
    <Card style={{padding:"12px 14px",marginBottom:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(220px,100%),1fr))",gap:10,alignItems:"end"}}>
        <div>
          <div style={{fontSize:11,color:C.ink4,fontWeight:800,marginBottom:5,fontFamily:"'Sarabun',sans-serif",letterSpacing:.3,textTransform:"uppercase"}}>ค้นหา</div>
          <div style={{position:"relative"}}><span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)"}}><Ic d={I.search} s={14} c={C.ink4}/></span><input value={q} onChange={e=>setQ(e.target.value)} placeholder="ชื่อวัตถุดิบ / หมวด..." style={{...iS,paddingLeft:34,fontSize:13,padding:"8px 12px 8px 34px"}}/></div>
        </div>
        <div>
    ...
    {grouped.length===0?<Card style={{padding:"50px 20px",textAlign:"center"}}>
      <div style={{fontSize:48,marginBottom:8,opacity:.6}}>📭</div>
      <div style={{fontFamily:"'Sarabun',sans-serif",fontSize:14,color:C.ink3,fontWeight:700}}>ไม่พบวัตถุดิบในเงื่อนไขนี้</div>
    </Card>:<div style={{display:"flex",flexDirection:"column",gap:12,paddingBottom:80}}>
      {grouped.map(g=>{
      ...
      <div style={{position:"fixed",left:isMobile?0:240,right:0,bottom:0,zIndex:80,background:"linear-gradient(135deg,#0F172A 0%,#1E293B 70%,#0F172A 100%)",borderTop:`2px solid ${C.brand}`,boxShadow:"0 -6px 18px rgba(15,23,42,0.30)",paddingBottom:"env(safe-area-inset-bottom,0)"}}>
        <div style={{maxWidth:1600,margin:"0 auto",padding:isMobile?"8px 12px":"10px 18px",display:"flex",alignItems:"center",flexDirection:isMobile?"column":"row",gap:isMobile?8:14}}>
          <div style={{flex:isMobile?"none":"1 1 0",width:isMobile?"100%":"auto",display:"flex",alignItems:"center",gap:isMobile?8:14,flexWrap:"wrap"}}>
            ...
            <div style={{display:"inline-flex",alignItems:"baseline",gap:6,whiteSpace:"nowrap"}}>
              <span style={{fontSize:isMobile?10:11,color:"#A7F3D0",fontFamily:"'Sarabun',sans-serif",fontWeight:700}}>ประมาณการ</span>
              <span style={{fontSize:isMobile?14:16,color:"#A7F3D0",fontFamily:"'Sarabun',sans-serif",fontWeight:900}}>฿{totals.totalCost.toLocaleString(undefined,{minimumFractionDigits:2})}</span>
            </div>
          </div>
...
// "หมวดหมู่" — manage categories AND assign menus into them. Click a category →
// tick the menus that belong to it. A menu lives in exactly ONE category, so menus
// already in another category are shown disabled here.
    ...
    {loading?<Loading text="โหลดหมวดหมู่..."/>:<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(260px,100%),1fr))",gap:16}}>
      ...
      <div>
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          <input value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="ชื่อหมวดใหม่ เช่น ของทอด" style={{...iS,flex:1}}/>
          <Btn onClick={add} loading={busy} icon={I.plus}>เพิ่ม</Btn>
        </div>
        {cats.length===0?<div style={{textAlign:"center",padding:30,color:C.ink4,fontFamily:"'Sarabun',sans-serif",fontSize:13}}>ยังไม่มีหมวดหมู่ — เพิ่มด้านบน</div>
        :<div style={{display:"flex",flexDirection:"column",gap:6,maxHeight:"52vh",overflowY:"auto"}}>
          {cats.map(c=>{const n=menus.filter(m=>catOf(m)===c.name).length;const on=selCat===c.name;return <div key={c.id} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",border:`1.5px solid ${on?C.brand:C.line}`,borderRadius:10,background:on?C.brandLight:C.white}}>
            {editId===c.id?<>
              <input value={editName} onChange={e=>setEditName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&rename()} autoFocus style={{...iS,flex:1}}/>
              <Btn onClick={rename} loading={busy} s={{padding:"5px 10px",fontSize:11}}>✓</Btn>
              <Btn v="ghost" onClick={()=>{setEditId(null);setEditName("");}} s={{padding:"5px 10px",fontSize:11}}>✕</Btn>
            </>:<>
              <button onClick={()=>setSelCat(c.name)} style={{flex:1,textAlign:"left",background:"none",border:"none",cursor:"pointer",fontFamily:"'Sarabun',sans-serif",fontSize:14,fontWeight:on?800:700,color:on?C.brand:C.ink}}>📂 {c.name} <span style={{fontSize:11,color:C.ink4,fontWeight:600}}>({n})</span></button>
              <button onClick={()=>{setEditId(c.id);setEditName(c.name);}} style={{background:C.blueLight,border:"none",borderRadius:7,padding:"5px 9px",cursor:"pointer",fontSize:11,fontWeight:700,color:C.blue,fontFamily:"'Sarabun',sans-serif"}}>แก้</button>
              <button onClick={()=>del(c)} style={{background:C.redLight,border:"none",borderRadius:7,padding:"5px 9px",cursor:"pointer",fontSize:11,fontWeight:700,color:C.red,fontFamily:"'Sarabun',sans-serif"}}>ลบ</button>
            </>}
          </div>;})}
        </div>}
      </div>
      {/* Right: assign menus to the selected category */}
      <div>
        {!selCat?<div style={{padding:"40px 16px",textAlign:"center",color:C.ink4,fontFamily:"'Sarabun',sans-serif",border:`1px dashed ${C.line}`,borderRadius:12}}><Ic d={I.tag} s={40} c={C.line}/><p style={{marginTop:8,fontSize:13}}>คลิกหมวดทางซ้าย แล้วเลือกเมนูที่จะเพิ่มเข้าหมวดนั้น</p></div>
        :<>
          <div style={{fontFamily:"'Sarabun',sans-serif",fontSize:14,fontWeight:900,color:C.ink,marginBottom:8}}>เมนูในหมวด "{selCat}"</div>
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="ค้นหาเมนู..." style={{...iS,marginBottom:8}}/>
          <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:"44vh",overflowY:"auto"}}>
            {visibleMenus.map(m=>{const cur=catOf(m);const here=cur===selCat;const elsewhere=cur&&cur!==selCat;return <label key={m.id} title={elsewhere?`อยู่ในหมวด "${cur}" แล้ว`:""} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:9,cursor:elsewhere?"not-allowed":"pointer",background:here?C.brandLight:C.white,border:`1px solid ${here?C.brandBorder:C.line}`,opacity:elsewhere?.55:1}}>
              <input type="checkbox" checked={here} disabled={elsewhere} onChange={()=>toggleMenu(m,selCat)} style={{accentColor:C.brand,width:16,height:16,flexShrink:0}}/>
              <span style={{flex:1,fontFamily:"'Sarabun',sans-serif",fontSize:13.5,fontWeight:here?800:600,color:here?C.brand:C.ink,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{m.name}</span>
              {elsewhere&&<span style={{fontSize:10.5,color:C.ink4,fontFamily:"'Sarabun',sans-serif",whiteSpace:"nowrap"}}>อยู่ใน {cur}</span>}
            </label>;})}
            {visibleMenus.length===0&&<div style={{padding:20,textAlign:"center",color:C.ink4,fontSize:12,fontFamily:"'Sarabun',sans-serif"}}>ไม่พบเมนู</div>}
          </div>
        </>}
      </div>
    </div>}
