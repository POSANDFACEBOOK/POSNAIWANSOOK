...
const OTHER_CHILD_LINES = [
  ...
  // ช่องทางที่พนักงานกด "อื่นๆ (ไม่ระบุ)" — ต้องมีบรรทัดของตัวเองเสมอ เพราะพ่อ Custom Payment
  // ที่มีลูกจะไม่ถูกนับในกติกา leaf ของฝั่งบัญชี ⟹ เงินก้อนนี้จะตกจากการแตกหมวดรายได้
  { pm: "other", name_th: "ช่องทางอื่น (ไม่ระบุ)", name_en: "ช่องทางอื่น (ไม่ระบุ)" },
];
function buildPaymentLines(bills) {
  ...
  // ประตูที่ปิดไปแล้ว: ช่องทาง "อื่นๆ (ไม่ระบุ)" มีบรรทัดลูกของตัวเองแล้ว (ดู OTHER_CHILD_LINES)
  // ประตูที่ยังเปิดอยู่: ถ้าวันไหนมีบิลบัตรเครดิต/เดบิตปนกับพร้อมเพย์ พ่อ "บัตรเครดิต (กรอกเอง)"
  //   จะมีลูกแค่พร้อมเพย์ ⟹ ยอดบัตรหายจากหมวดรายได้ · วันนี้จอขายไม่มีช่องทางบัตรจึงยังไม่เกิด
  //   ถ้าจะเพิ่มช่องทางบัตรเมื่อไหร่ ต้องคุยกับฝั่งบัญชีให้เปิดหมวด Visa ของสาขาก่อน แล้วค่อยเพิ่มบรรทัดลูก
    ...
    try {
      ...
      const mkDrawer = (bills) => {
        ...
        return {
          ...
          // แยก "เงินใส่เข้าลิ้นชัก" กับ "เงินหยิบออก" ให้ฝั่งบัญชีด้วย ไม่ใช่ส่งแต่ยอดสุทธิ
          // เหตุจริง 20 ก.ย. 69: เงินทอนตั้งต้นที่ใส่ทีหลัง (13 ก.ย. The River ฿2,000) ถูกกดเป็น "ฝากเงิน"
          // สูตรตู้เซฟของเขาคือ expected − start จึงฝากเงินทอนเข้าตู้เซฟเป็นรายรับไปด้วย
          // เขาไล่ทั้งฐานแล้วพบว่าฝากเกินจริงรวม ฿18,653 (หนักสุด 21 ส.ค. บางใหญ่ ฿6,687 ที่วันนั้นขายเงินสด ฿0)
          // วันที่มีทั้งเข้าและออกปนกัน ยอดสุทธิตัวเดียวหักไม่ได้ถูก ⟹ ต้องแยกช่อง
          paid_in: payIn,                      // เงินใส่เข้าลิ้นชัก (เงินทอนเพิ่ม ฯลฯ) — ไม่ใช่รายรับ
